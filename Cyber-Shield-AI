...
result.add_flag(f"Attachment uses double extension: {name}", 20)
...
recs.append("Verify the sender using an official website or phone number.")
recs.append("Do not enter passwords through links in this email.")
recs.append("Hover over links before clicking.")
else:
recs.append("No strong phishing signs were detected.")
recs.append("Still verify unexpected requests for money or login information.")

if result.attachments:
recs.append("Be extra careful with attachments, especially archives or executable files.")
